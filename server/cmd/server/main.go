...
func main() {
	...
	licenseLeaseTTL, err := time.ParseDuration(cfg.LicenseLeaseTTL)
	if err != nil {
		log.Fatalf("LICENSE_LEASE_TTL: %v", err)
	}
	retainedLicenseKeys, err := license.ParseRetainedPublicKeys(cfg.LicenseRetainedPublicKeys)
	if err != nil {
		log.Fatalf("LICENSE_RETAINED_PUBLIC_KEYS: %v", err)
	}
	if _, exists := retainedLicenseKeys[cfg.LicenseSigningKeyID]; exists {
		log.Fatalf("LICENSE_RETAINED_PUBLIC_KEYS must not contain active key id %q", cfg.LicenseSigningKeyID)
	}
	licenseSvc := service.NewLicenseService(db, licenseSigningPriv, cfg.LicenseSigningKeyID, retainedLicenseKeys, licenseLeaseTTL, logger, bf, webhookSvc)
...
<script src="https://cdn.jsdelivr.net/npm/@scalar/api-reference"></script>
	...
	licensePubBase64 := base64.StdEncoding.EncodeToString(licenseSvc.SigningPublicKey())
	licenseKeyID := licenseSvc.SigningKeyID()
	licensePublicKeyMap := gin.H{}
	licensePublicKeyDetails := make([]gin.H, 0)
	allLicenseKeys := licenseSvc.SigningPublicKeys()
	licenseKeyIDs := make([]string, 0, len(allLicenseKeys))
	for kid := range allLicenseKeys {
		licenseKeyIDs = append(licenseKeyIDs, kid)
	}
	sort.Strings(licenseKeyIDs)
	for _, kid := range licenseKeyIDs {
		pub := allLicenseKeys[kid]
		pubHex := hex.EncodeToString(pub)
		pubBase64 := base64.StdEncoding.EncodeToString(pub)
		licensePublicKeyMap[kid] = pubBase64
		licensePublicKeyDetails = append(licensePublicKeyDetails, gin.H{
			"kid": kid, "algorithm": "ed25519", "public_key": pubHex,
			"public_key_base64": pubBase64, "format": "hex",
		})
	}
	v1.GET("/license/pubkey", func(c *gin.Context) {
		...
		response.OK(c, gin.H{
			...
			"public_key_base64": licensePubBase64,
			"format":            "hex",
			"keys":              licensePublicKeyDetails,
		})
	})
