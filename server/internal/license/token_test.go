...
func TestSignAndVerify(t *testing.T) {
	...
	if len(parsed.Entitlements) != 1 || parsed.Entitlements[0] != "export" {
		t.Errorf("Entitlements mismatch: %v", parsed.Entitlements)
	}
	if parsed.Nonce == "" {
...
func TestVerifyWithKeyRing(t *testing.T) {
	current := testKey(t)
	old := testKey(t)
	tok := &VerifyToken{KeyID: "old-2025", LicenseID: "lic", IssuedAt: 1, ExpiresAt: 9999999999}
	raw, err := Sign(tok, old)
	if err != nil {
		t.Fatal(err)
	}
	parsed, err := VerifyWithKeyRing(raw, map[string]ed25519.PublicKey{
		"current-2026": PublicKey(current),
		"old-2025":     PublicKey(old),
	})
	if err != nil || parsed.KeyID != "old-2025" {
		t.Fatalf("key-ring verify failed: token=%+v err=%v", parsed, err)
	}
	if _, err := VerifyWithKeyRing(raw, map[string]ed25519.PublicKey{"current-2026": PublicKey(current)}); err == nil {
		t.Fatal("expected unknown kid error")
	}
}

func TestParseRetainedPublicKeys(t *testing.T) {
	pub := PublicKey(testKey(t))
	encoded := base64.StdEncoding.EncodeToString(pub)
	ring, err := ParseRetainedPublicKeys(fmt.Sprintf(`{"old-2025":%q}`, encoded))
	if err != nil || len(ring) != 1 || !ring["old-2025"].Equal(pub) {
		t.Fatalf("valid retained ring rejected: ring=%v err=%v", ring, err)
	}

	bad := []string{
		fmt.Sprintf(`{"old":%q,"old":%q}`, encoded, encoded),
		`{"bad key":"AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA="}`,
		`{"old":"not-base64"}`,
		`{"old":"YQ=="}`,
		fmt.Sprintf(`{"old":%q} trailing`, encoded),
	}
	for _, raw := range bad {
		if _, err := ParseRetainedPublicKeys(raw); err == nil {
			t.Errorf("expected retained ring to reject %s", raw)
		}
	}
}
