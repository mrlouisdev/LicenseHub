...
func (s *LicenseService) RefreshLease(ctx context.Context, in LeaseInput) (string, error) {
	lic, err := s.licenseFromLease(ctx, in)
	if err != nil {
		return "", err
	}
	...
	return s.signToken(lic, in.Identifier)
}

func (s *LicenseService) DeactivateLease(ctx context.Context, in LeaseInput) error {
	lic, err := s.licenseFromLease(ctx, in)
	if err != nil {
		return err
	}
...
func (s *LicenseService) licenseFromLease(ctx context.Context, in LeaseInput) (*model.License, error) {
	t, err := s.verifyLeaseBinding(in)
	if err != nil {
		return nil, licenseNotFound()
	}
	lic, err := s.store.FindLicenseByID(ctx, t.LicenseID)
	if err != nil || lic.ProductID != in.ProductID {
		return nil, licenseNotFound()
	}
...
func (s *LicenseService) verifyLeaseBinding(in LeaseInput) (*license.VerifyToken, error) {
	t, err := license.VerifyWithKeyRing(in.Lease, s.verificationKeys)
	if err != nil || t.Version != 1 || t.ProductID != in.ProductID || t.Identifier != in.Identifier {
		return nil, licenseNotFound()
	}
	return t, nil
}
...
func NewLicenseService(s *store.Store, signingKey licenseSigningKey, signingKeyID string, verificationKeys map[string]ed25519.PublicKey, leaseTTL time.Duration, logger *slog.Logger, failures FailureTracker, webhook *WebhookService) *LicenseService {
	if leaseTTL <= 0 {
		leaseTTL = 72 * time.Hour
	}
	keys := make(map[string]ed25519.PublicKey, len(verificationKeys)+1)
	for kid, pub := range verificationKeys {
		keys[kid] = append(ed25519.PublicKey(nil), pub...)
	}
	keys[signingKeyID] = license.PublicKey(signingKey)
	return &LicenseService{store: s, signingKey: signingKey, signingKeyID: signingKeyID, verificationKeys: keys, leaseTTL: leaseTTL, logger: logger, failures: failures, webhook: webhook}
}
...
func (s *LicenseService) SigningKeyID() string { return s.signingKeyID }

func (s *LicenseService) SigningPublicKeys() map[string]ed25519.PublicKey {
	keys := make(map[string]ed25519.PublicKey, len(s.verificationKeys))
...
func (s *LicenseService) entitlementNames(lic *model.License) []string {
	if lic.Plan == nil {
		return []string{}
	}
	features := make([]string, 0, len(lic.Plan.Entitlements))
	for _, e := range lic.Plan.Entitlements {
		if e.ValueType == "bool" && e.Value != "true" {
			continue
		}
...
func (s *LicenseService) signToken(lic *model.License, identifier string) (string, error) {
	...
	t := &license.VerifyToken{
		...
		Entitlements: s.entitlementNames(lic),
		IssuedAt:     now.Unix(),
		ExpiresAt:    now.Add(s.leaseTTL).Unix(),
		GraceDays:    s.graceDays(lic),
